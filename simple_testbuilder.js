#!/usr/bin/env node
/**
 * Simple TestBuilder - Walks around, survives, and actually works!
...
bot.on('spawn', () => {
  console.log(`[TestBuilder] 🚀 TestBuilder spawned successfully!`);
  stats.sessions++;
  ...
  actionInterval = setInterval(async () => {
    try {
      ...
      console.log(`[TestBuilder] ❌ Activity error: ${err.message}`);
    }
  ...
  setInterval(() => {
    ...
    console.log(`[TestBuilder] 📊 Health: ${bot.health}/20, Food: ${bot.food}/20, Playtime: ${playtime}m`);
  }, 30000);
});
...
async function doRandomActivity() {
  // Health check first
  if (bot.health <= 6) {
    console.log(`[TestBuilder] ❤️ Low health! Looking for food...`);
  ...
  // Food check
  if (bot.food <= 8) {
    console.log(`[TestBuilder] 🍞 Hungry! Looking for food...`);
...
async function exploreArea() {
  console.log(`[TestBuilder] 🗺️ Exploring new area...`);
  ...
  try {
    ...
    console.log(`[TestBuilder] ✅ Explored to (${Math.round(randomX)}, ${Math.round(randomZ)})`);
    
    if (Math.random() < 0.3) {
      bot.chat(`🗺️ Exploring at coordinates ${Math.round(randomX)}, ${Math.round(randomZ)}!`);
    }
  } catch (err) {
    console.log(`[TestBuilder] 🚶 Exploration interrupted: ${err.message}`);
  }
}

async function chopWood() {
  console.log(`[TestBuilder] 🪓 Looking for wood...`);
  ...
  try {
    ...
    if (tree) {
      ...
      await bot.dig(tree);
      console.log(`[TestBuilder] ✅ Chopped ${tree.name}!`);
    ...
    console.log(`[TestBuilder] 🪓 Wood chopping failed: ${err.message}`);
  }
}

async function digStone() {
  console.log(`[TestBuilder] ⛏️ Looking for stone...`);
  ...
  try {
    ...
    if (stone) {
      ...
      await bot.dig(stone);
      console.log(`[TestBuilder] ✅ Mined ${stone.name}!`);
    }
  } catch (err) {
    console.log(`[TestBuilder] ⛏️ Stone mining failed: ${err.message}`);
  }
}

async function buildSimpleStructure() {
  console.log(`[TestBuilder] 🔨 Building something simple...`);
  ...
  try {
    ...
    if (material && material.count > 5) {
      ...
      console.log(`[TestBuilder] ✅ Built small platform with ${material.name}!`);
    ...
    console.log(`[TestBuilder] 🔨 Building failed: ${err.message}`);
  }
}

async function huntAnimals() {
  console.log(`[TestBuilder] 🏹 Looking for animals...`);
  
  try {
    const animals = Object.values(bot.entities).filter(entity => {
      return entity.displayName && 
             ['Pig', 'Cow', 'Sheep', 'Chicken'].includes(entity.displayName) &&
             bot.entity.position.distanceTo(entity.position) <= 16;
    });
    
    if (animals.length > 0) {
      const target = animals[0];
      console.log(`[TestBuilder] 🎯 Found ${target.displayName}!`);
      
      await bot.pathfinder.goto(new goals.GoalFollow(target, 1));
      await bot.attack(target);
      
      console.log(`[TestBuilder] ✅ Attacked ${target.mobType}!`);
    ...
    console.log(`[TestBuilder] 🏹 Hunting failed: ${err.message}`);
  }
}

async function collectNearbyItems() {
  console.log(`[TestBuilder] 📦 Looking for dropped items...`);
  ...
  try {
    ...
    if (items.length > 0) {
      ...
      console.log(`[TestBuilder] ✅ Collected dropped item!`);
    }
  } catch (err) {
    console.log(`[TestBuilder] 📦 Item collection failed: ${err.message}`);
  }
}

async function findAndEatFood() {
  console.log(`[TestBuilder] 🍽️ Looking for food in inventory...`);
  ...
  try {
    ...
    if (food) {
      ...
      await bot.consume();
      console.log(`[TestBuilder] ✅ Ate ${food.name}! Health: ${bot.health}, Food: ${bot.food}`);
      
      if (Math.random() < 0.3) {
        bot.chat(`🍽️ Yum! Ate some ${food.name}!`);
      }
    } else {
      console.log(`[TestBuilder] ❌ No food in inventory - continuing activities`);
      // Don't crash, just continue other activities
      await huntAnimals(); // Try to hunt for food
    }
  } catch (err) {
    console.log(`[TestBuilder] 🍽️ Eating failed: ${err.message}`);
...
// Handle death
bot.on('death', () => {
  console.log(`[TestBuilder] 💀 TestBuilder died! Respawning...`);
  stats.deaths++;
...
bot.on('end', () => {
  console.log(`[TestBuilder] 🔌 Disconnected. Attempting to reconnect...`);
  if (actionInterval) clearInterval(actionInterval);
});

bot.on('error', (err) => {
  console.log(`[TestBuilder] 🚨 Bot error: ${err.message}`);
...
setInterval(() => {
  if (Math.random() < 0.4) { // 40% chance every 20 seconds - very active!
    const messages = [
      "🎮 Hey everyone! TestBuilder here - 24/7 survival mode active!",
...
console.log(`[TestBuilder] 🚀 Starting Simple TestBuilder - 24/7 Survival Mode!`);